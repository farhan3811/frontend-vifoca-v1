...
const PenilaianList = () => {
  ...
  const API_URL = process.env.NODE_ENV === 'production'
    ...
    const getPenilaian = async () => {
      try {
        console.log("Fetching penilaian with params:", { search });
          ...
          {penilaian.length > 0 ? (
            penilaian.map((item, index) => (
              ...
              >
                ...
                <td className="p-4">{item.userId || "N/A"}</td>
                <td className="p-4">{item.materi_id || "N/A"}</td>
                <td className="p-4">{item.tugas_id || "N/A"}</td>
