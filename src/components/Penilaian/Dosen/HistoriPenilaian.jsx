...
const PenilaianList = () => {
  ...
  const TABLE_HEAD = ["No", "Nama Mahasiswa", "Materi", "Latihan", "Tanggal","Nilai", "Aksi"];
    ...
    <Card className="w-full h-full mb-4">
      <table className="w-full min-w-max table-auto text-left">
        <thead>
          <tr>
            {TABLE_HEAD.map((head) => (
              <th key={head} className="border-b border-blue-gray-100 bg-blue p-4">
                <Typography variant="small" color="white" className="font-normal leading-none font-title">
                  {head}
                </Typography>
              </th>
            ))}
        ...
        <tbody>
          {penilaian.length > 0 ? (
            penilaian.map((item, index) => (
              <tr key={item.id || index} className={index % 2 === 0 ? "bg-odd" : "bg-white"}>
                ...
                <td className="p-4">{item.createdat ? new Date(item.createdat).toLocaleDateString() : "N/A"}</td>
                <td className="p-4">{item.form_penilaian}</td>
      ...
      <DetailModal penilaian={selectedPenilaian} open={openDetailModal} onClose={handleCloseDetailModal} />
    </Card>
  );
};
